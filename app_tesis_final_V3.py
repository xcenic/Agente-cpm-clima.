...
# --- PILAR IA 3 (15%): AGENTE PRESCRIPTIVO (Algoritmo Evolutivo/Heurístico) ---
def agente_prescriptivo_mitigacion(df_tareas, evb_total):
    ...
    # Búsqueda Heurística: Identificar nodos bloqueados (Tr alto) y Nodos Refugio (Tr bajo)
    ...
    if not sugerencias and not bloqueadas.empty:
        sugerencias.append("⚠️ **Alerta de Parálisis Sistémica:** Múltiples frentes bloqueados por lodo, pero no se detectaron 'Nodos Refugio' simultáneos para reasignar la flotilla.")
...
def simular_cronograma(df, clima, prob_min, mm_min, dias_idx, feriados, reparar, umbral_horas, h_inicio, h_fin, use_nlp, use_ml, temp_global, hum_global):
    ...
    for tid in orden:
        ...
        # ---------------------------------------------------------
        # INYECCIÓN IA MULTIVARIABLE: NLP + Machine Learning 
        # ---------------------------------------------------------
        ic_base = calcular_ic_ia(row['Name'], use_nlp)
        tr_horas, impacto_constructivo_ic = calcular_tr_y_ic_dinamico(mm_min, temp_global, hum_global, ic_base, horas_jornada, use_ml)
        ...
        if not row['IsSummary'] and not row['IsMilestone'] and new_start:
            ...
            if retraso_teorico_dias > 0:
                ...
                # Q: Cuantización a medios días
                total_cuantizado = math.ceil(duracion_total_teorica * 2) / 2
                
                # Ut: Umbral de horas operativas
                fraccion = total_cuantizado % 1
                horas_trabajadas = horas_jornada if fraccion == 0 else (fraccion * horas_jornada)
